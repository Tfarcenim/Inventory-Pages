...
public class DropOffConfig {
    ...
    public static void onConfigChanged(ModConfigEvent event) {
        if (!event.getConfig().getModId().equals(InventoryPages.MOD_ID)) {
            return;
        }
        ...
        blockEntityBlacklist = Client.blacklistedTes.get()
                ...
                .map(ResourceLocation::new).filter(resourceLocation -> {
                    ...
                    if (!b) {
                        InventoryPages.LOG.warn("Ignoring unknown blockentity: " + resourceLocation);
                    }
                    return b;
                })
        ...
        InventoryPages.LOG.info("Configuration changed.");
    }
}
